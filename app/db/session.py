...
def _build_engine(database_url: str) -> Engine:
    ...
    SQLite remains available for tests and disposable local demos. PostgreSQL
    receives connection health checks and bounded pooling so a long-running
    FastAPI process can recover from a restarted database without silently
    handing out stale connections.
    """
    ...
    if is_sqlite:
        ...
        if normalized_url.startswith("sqlite:///") and not normalized_url.endswith(":memory:"):
            raw_path = normalized_url.removeprefix("sqlite:///")
            if raw_path:
                Path(raw_path).expanduser().parent.mkdir(parents=True, exist_ok=True)
    elif is_postgres or is_cockroachdb:
        # These settings apply only to queue-pooled network databases.
        # pool_pre_ping is disabled because each ping adds ~70ms to CockroachDB
        # Cloud connections.  pool_recycle handles stale connections instead.
...
def warmup_pool() -> None:
    ...
    try:
        ...
        print(f"DB pool warmed up in {time.time()-t0:.2f}s")
    except Exception as e:
        print(f"DB warmup failed after {time.time()-t0:.2f}s: {e}")
